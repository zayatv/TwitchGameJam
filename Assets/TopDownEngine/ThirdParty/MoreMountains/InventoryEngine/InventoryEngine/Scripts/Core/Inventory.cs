...
;
		...
		public virtual void EquipItem(InventoryItem item, int index, InventorySlot slot = null)
		{
			if (InventoryType == Inventory.InventoryTypes.Main)
			{
				...
				if (item.TargetEquipmentInventory(PlayerID).Content.Length == 2 && item.TargetEquipmentInventoryName == "KoalaWeaponInventory")
				{
                    if (!InventoryItem.IsNull(item.TargetEquipmentInventory(PlayerID).Content[1]))
                    {
                        if (
                            (item.CanSwapObject)
                            && (item.TargetEquipmentInventory(PlayerID).Content[1].CanMoveObject)
                            && (item.TargetEquipmentInventory(PlayerID).Content[1].CanSwapObject)
                        )
                        {
                            // we store the item in the equipment inventory
                            oldItem = item.TargetEquipmentInventory(PlayerID).Content[1].Copy();
                            oldItem.UnEquip(PlayerID);
                            MMInventoryEvent.Trigger(MMInventoryEventType.ItemUnEquipped, slot, this.name, oldItem, oldItem.Quantity, index, PlayerID);
							item.TargetEquipmentInventory(PlayerID).RemoveItem(1, oldItem.Quantity);
                        }
                    }
                }
