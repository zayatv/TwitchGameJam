...
namespace MoreMountains.TopDownEngine
{
	...
	public class CharacterInventory : CharacterAbility, MMEventListener<MMInventoryEvent>
	{
        ...
        protected virtual void SwitchWeapon()
		{
            ...
            if (WeaponInventory.Content[1] == null)
				return;

            InventoryWeapon[] inventoryWeapons = new InventoryWeapon[WeaponInventory.Content.Length];

			for (int i = 0; i < inventoryWeapons.Length; i++)
			{
				inventoryWeapons[i] = (InventoryWeapon) WeaponInventory.Content[i];
            }

            if (CharacterHandleWeapon.CurrentWeapon.WeaponID == inventoryWeapons[0].ItemID)
			{
                CharacterHandleWeapon.ChangeWeapon(inventoryWeapons[1].EquippableWeapon, inventoryWeapons[1].ItemID, false);
            }
			else
			{
                CharacterHandleWeapon.ChangeWeapon(inventoryWeapons[0].EquippableWeapon, inventoryWeapons[0].ItemID, false);
            }
				...
				if (isClass || isSubclass)
				{
					...
					switch (inventoryWeapon.AutoEquipMode)
					{
						...
						case InventoryWeapon.AutoEquipModes.AutoEquipIfEmptyHanded:
							...
							break;
						case InventoryWeapon.AutoEquipModes.AutoEquipIfEmptySlot:
							if (WeaponInventory.Content[1] == null)
							{
                                _nextFrameWeapon = true;
                                _nextFrameWeaponName = inventoryEvent.EventItem.ItemID;
